...
ForEach ($file in Get-ChildItem -Path $sourceDir -Name -Include @("*.mkv")) {
    ...
    ffmpeg -y -loglevel error -hide_banner -stats -r $fps -i "$sourceDir\$file" -preset veryslow -b:v 0 -crf 15 -max_interleave_delta 0 -vf mpdecimate -fps_mode vfr -an -max_muxing_queue_size 9999 $temp\Fast$file
}
...
ffmpeg -y -loglevel error -stats -f concat -safe 0 -i ${temp}\list.txt -codec copy .\temp\Concatenated.mkv
