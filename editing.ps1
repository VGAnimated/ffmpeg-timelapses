...
function checkSubtitleFile {
    ...
    # Check if the first Artist - Title pair is empty
    if ($json.Artists[0] -eq "" || $json.Titles[0] -eq "") {
        throw "Subtitle files has empty values, aborting"
    }
    ...
    $json.Artists.GetEnumerator() | ForEach-Object {
        ...
        foreach($banned in @(',', '°', '(', ')','[',']', '"',"{", "}",'ä', 'ü', 'ö', 'ê', 'é', ":", ";", "'")) {
            if ($artist.IndexOf($banned) -ne -1) {
                throw "<$banned> character present in $artist"
            }
            if ($title.IndexOf($banned) -ne -1) {
                throw "<$banned> character present in $title"
            }
        }
    }
}
...
function EditVideo {
    ...
    if (!(Test-Path($image))) {
        ...
        $answer = Read-Host "Press any key to exit"
        exit
    }
    ...
    if (!(Test-Path .\config\videos.txt)) {
        New-Item -ItemType File .\config\videos.txt
    }
    ...
    # region Duplicated frames count
    if (!(Test-Path .\config\duplicates.txt)) {
        New-Item -ItemType File .\config\duplicates.txt
    }
    ...
    # Length after a 15x acceleration
    $videoShort = $videoLength/15
    ...
    if (!(Test-Path .\config\audios.txt)) {
        New-Item -ItemType File .\config\audios.txt
    }
    ...
    Write-Host "Audio length : $audioLength seconds or $([math]::Floor($audioLength/60))m$($audioLength%60)s" -ForegroundColor Magenta
    # endregion
    ...
    if (($audioLength -gt ($videoLength/20) + 20) -or $audioLength -eq 0)
    {
        ...
        $answer = Read-Host "Do you wish to continue? (y/N)"
        if ($answer -ne "y") {
            exit
        }
    }
    ...
    & $speedUpScript -sourceDir "$videoFolder" -temp ".\temp\speedup" -pts 15 -fps 60 -del "Y"
    ...
    Write-Host "Video length post treatment = $duration seconds or $([math]::Floor($duration/60))m$($duration%60)s" -ForegroundColor Magenta
    ...
    Write-Host -ForegroundColor Magenta "Target duration: $target seconds or $([math]::Floor($target/60))m$($target%60)s"
    ...
    ffmpeg-bar -y -loglevel info -hide_banner -stats -i $video -i .\images\wm.png -crf 15 -preset veryslow -filter_complex "
    [1:v]
    scale=-1:200,
